...
bool forward_propagate(NeuralNetwork* network, const double* targets, size_t num_targets) {
    // Validate input parameters
    if(network == NULL || targets == NULL) {
        fprintf(stderr, "Error: Null pointer passed to forward_propagate.\n");
        return false;
    }
    ...
    if(num_targets != network->layer_sizes[output_layer]) {
        fprintf(stderr, "Error: Number of targets does not match output layer size.\n");
        return false;
    }
